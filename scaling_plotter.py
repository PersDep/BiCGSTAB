...
for size in sizes:
    for i in nodes_amount:
        data = open("result_mpi_" + str(i) + '_' + str(size) + "_scaling.out", "r").readlines()
        time_type = 0
        for line in data:
...
for size, types in sorted(acceleration_table.items()):
    ...
    for type, acceleration in sorted(types.items()):
        ...
        if type == 0:
            plotter.plot(nodes_amount, acceleration, colors[color_index], label="Size: from " + str(size) + " to " + str(size * 8), linewidth='1')
            for i, nodes in enumerate(nodes_amount):
                if nodes != 1:
                    plotter.text(nodes + 0.04, acceleration[i] + 0.02, str(size * nodes), color=colors[color_index])
...
for i, tick in enumerate(acceleration_ticks):
    if i > 0 and tick - acceleration_ticks[i - 1] < 0.02:
